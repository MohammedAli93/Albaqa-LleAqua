...
export function Winner() {
  ...
  return (
    <div className="safe relative grid h-dvh place-items-center overflow-hidden p-5 lg:h-full lg:p-8">
      <HostBg variant="sky" />
      <ConfettiRain />
  ...
  return (
    ...
    >
      <motion.div animate={{ y: [0, -10, 0], rotate: [-3, 3, -3] }} transition={{ duration: 3, repeat: Infinity }}>
        <Trophy className="h-[1.05em] w-[1.05em] text-[clamp(3rem,5.5vw,6rem)] text-[#F6C43E]" style={{ filter: 'drop-shadow(0 0 36px rgba(246,196,62,0.9))' }} />
      </motion.div>
...
/** Stage 1 — the champion celebration (reference screen 23, big-screen scale). */
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col items-center gap-3 text-center lg:gap-5">
      ...
      {isTeams && team ? (
        <>
          ...
          <p className="tnum font-display text-screen-score font-black text-[#FFE9A8] drop-shadow"><CountUp value={team.score} /> {t(L, 'points')}</p>
        </>
      ) : winner ? (
        <>
          ...
          {isElim ? (
            ...
            <p className="tnum font-display text-screen-score font-black text-[#FFE9A8] drop-shadow"><CountUp value={winner.score} /> {t(L, 'points')}</p>
          )}
        </>
      ) : null}
        ...
        {rows.map((e, i) => {
          ...
          return (
            <LeaderRow
              ...
              value={isElim ? (out ? <Skull size={24} /> : <Hearts lives={e.lives} size={24} />) : (champ ? <CountUp value={e.score} /> : e.score)}
            />
          );
        })}
      </div>
...
function TeamResult({
  teams, leaderboard, winnerTeam, championId,
}: {
  teams: TeamPublic[];
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3 lg:gap-4">
                ...
                {champ && <Crown className="shrink-0 text-[#FFE9A8]" style={{ filter: 'drop-shadow(0 0 12px rgba(255,233,168,0.8))' }} />}
                ...
                <span className="tnum shrink-0 font-display text-screen-score font-black">
                  {champ ? <CountUp value={team.score} /> : team.score} <span className="text-screen-meta font-bold text-white/80">{t(L, 'points')}</span>
                </span>
              </div>
              {members.length > 0 && (
